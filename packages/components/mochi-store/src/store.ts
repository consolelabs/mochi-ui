...
import { boringAvatar } from '@consolelabs/theme'

import { GetMochi } from './mochi'
import { ModelInAppWallet } from './type'
...
export type Profile = MochiProfile & {
  platformIcon: string
  name: string
  token: string
...
export const useMochi = create<MochiState>((set, get) => ({
  ...
  login: async (user, token, _addresses) => {
    ...
    // set access_token for mochiAPI
    GetMochi().mochiAPI.token(token)
    // Trigger load user profile
    const [p] = await GetMochi().UI.resolve(Platform.Web, user.id ?? '')
    const { ok, data } = await GetMochi().mochiAPI.pay.mochiWallet.getWallets(
      user.id ?? '',
    )
    ...
    set((state) => {
      const newState = {
        ...
        user: {
          ...user,
          avatar: user.avatar || boringAvatar(p?.plain),
          profile_name: (user.profile_name || p?.plain) ?? '',
          name: (user.profile_name || p?.plain) ?? '',
          platform: p?.platform ?? '',
          token,
          associatedAccounts,
          platformIcon:
      ...
      useWalletStore.getState().setWallets(user)
      return newState
    })
  },
  logout: () =>
    set((state) => {
      ...
      // remove access_token for mochiAPI
      GetMochi().mochiAPI.token('')
      localStorage.setItem('login-state', JSON.stringify(newState))
